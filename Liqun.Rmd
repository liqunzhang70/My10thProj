...
``` {r}
library(readr)
streaming <- read_csv("C:\\Users\\Admin\\Downloads\\streaming.csv")
#View(streaming)
ggplot(data = streaming) + geom_point(mapping = aes(x = Year, y = Age))
#ggplot(data = streaming) + geom_point(mapping = aes(x = Title, y = "Rotten Tomatoes"))
#ggplot(data = streaming) + geom_point(mapping = aes(x = Title, y = Directors ))
ggplot(data = streaming) + geom_histogram(mapping = aes(x = Year), bins = 15)
```


install tinytex by "install.packages("knitr") library(knitr) install.packages("tinytex") tinytex::install_tinytex()"
